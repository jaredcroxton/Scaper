"""
Ben's Bites Scraper
Scrapes recent articles from Ben's Bites newsletter via RSS feed.
"""
...
def fetch_rss() -> list[dict]:
    ...
    try:
        ...
        print(f"  [ERROR] Failed to fetch Ben's Bites RSS: {e}", file=sys.stderr)
        return []
...
if __name__ == "__main__":
    print("Ben's Bites Scraper - Starting...")
    results = scrape()
    print(f"Total Ben's Bites articles: {len(results)}")

    os.makedirs(".tmp", exist_ok=True)
    with open(".tmp/bens_bites_articles.json", "w") as f:
        json.dump(results, f, indent=2)
    print("Written to .tmp/bens_bites_articles.json")
